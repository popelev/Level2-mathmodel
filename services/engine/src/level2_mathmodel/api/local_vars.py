...
def create_local_var(request: Request, payload: dict[str, Any]) -> JSONResponse | dict[str, Any]:
    try:
...
@router.get("/{var_id}")
def get_local_var(request: Request, var_id: str) -> JSONResponse | dict[str, Any]:
    try:
...
@router.put("/{var_id}")
def replace_local_var(
    request: Request, var_id: str, payload: dict[str, Any]
