...
Config-driven polling: many Level2 **flag tags** can fire many different
...
3. **Poll scheduler** — asyncio background task (FastAPI lifespan) evaluates
   enabled triggers; errors are isolated per trigger.
...
| Env | Default | Meaning |
|-----|---------|---------|
| `RECALC_POLL_ENABLED` | off | `1` / `true` starts the scheduler |
| `POLL_INTERVAL_MS` | `1000` | Default poll interval |
...
When poll is enabled and the trigger store is empty, a default rule is seeded:
...
| Method | Path | Purpose |
|--------|------|---------|
| `GET` | `/api/v1/recalc/status` | Scheduler + last runs |
...
`recalc_poll_enabled`, `recalc_trigger_count`, `recalc_last_trigger_at`, etc.
...
4. Set `RECALC_POLL_ENABLED=1` and rebuild/restart the mathmodel container.
